...
library(readr)
demo_data <- read_csv(file = "demo_data.csv")
summary(demo_data)
```
...
qplot(x=isoweek(obs_date), y=mal_case, colour=as.factor(iso_year),
      ...
      main="Fogera Outpatient Malaria Cases", xlab="Iso Week", ylab = "Cases")
```
...
```{r, fig.height=3}
ggplot(data = mecha) +
  geom_point(mapping = aes(x = tot_case, y = mal_case, color = iso_year))
```
...
Now as we have converted our shapefile into a data frame containing the spatial information of our Woredas, we can add our epidemiological data to it. We will use `read_csv()` to load the epidemiological data. Then we use  `group_by()` and `summarize()` to summarize cases by Woreda. Finally we use `left_join()` to add our epidemiological data to the data frame of our shapefile. Tomorrow we will learn even more ways of summarizing data and of joining multiple dataset. 

```{r}
library(readr)
epi_dat <- read_csv ("epi_data_2017W42.csv")
...
1. Read the data file `epi_data_2017W42.csv`.
